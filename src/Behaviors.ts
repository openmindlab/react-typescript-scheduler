...
export const getSummary = (schedulerData: SchedulerData, headerEvents: Event[], slotId: string, slotName: string, headerStart: any, headerEnd: any) => {
...
export const getCustomDate = (schedulerData: SchedulerData, num: number, date: string = undefined) => {
    ...
    let startDate = num === 0 ? selectDate :
        moment(selectDate).add(2 * num, 'days').format(DATE_FORMAT),
        endDate = moment(startDate).add(1, 'days').format(DATE_FORMAT),
        cellUnit = CellUnits.Hour;
    if (viewType === ViewTypes.Custom1) {
        let monday = moment(selectDate).startOf('week').format(DATE_FORMAT);
        ...
        let firstDayOfMonth = moment(selectDate).startOf('month').format(DATE_FORMAT);
...
export const getDateLabel = (schedulerData:SchedulerData, viewType, startDate, endDate) => {
...
export const getScrollSpecialMoment = (schedulerData:SchedulerData, startMoment:moment.Moment, endMoment:moment.Moment) => {
    return endMoment;
}
