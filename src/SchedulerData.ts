...
export default class SchedulerData {
    ...
    public getContentCellWidth(): number {
        ...
        const schedulerWidth = parseInt(this.getSchedulerWidth(), undefined);
        return this.isContentViewResponsive() ? schedulerWidth * Number(contentCellConfigWidth.slice(0, -1)) / 100 : parseInt(contentCellConfigWidth, undefined);
    }
    ...
    private _createInitHeaderEvents(header: Header) {
        ...
        const endValue = this.showAgenda ? (this.viewType === ViewTypes.Week ? start.add(1, "weeks").format() : (
            this.viewType === ViewTypes.Day ? start.add(1, "days").format() : (
                this.viewType === ViewTypes.Month ? start.add(1, "months").format() : (
                    this.viewType === ViewTypes.Year ? start.add(1, "years").format() : (
                        this.viewType === ViewTypes.Quarter ? start.add(1, "quarters").format() :
                            moment(this.endDate).add(1, "days").format()
                    )
                )
            )
        )) : (this.cellUnit === CellUnits.Hour ? start.add(this.config.minuteStep, "minutes").format()
