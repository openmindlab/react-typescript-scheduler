...
interface EventItemProps {
    ...
    conflictOccurred?: (schedulerData: SchedulerData, name: string, evI: EventItem, ev: any, slotId: string, slotName: string, newStart: string, evItEnd: any) => any,
...
class EventItem extends Component<EventItemProps, EventItemState> {
    ...
    initStartDrag = (ev: any) => {
        const { schedulerData, eventItem } = this.props;
        let slotId = schedulerData.getEventSlotId(eventItem);
    ...
    stopStartDrag = (ev: any) => {
        ...
        if (count !== 0 && cellUnit !== CellUnits.Hour && config.displayWeekend === false) {
            if (count > 0) {
                ...
                while (true) {
                    i++;
                    let tempStart = moment(eventItem.start).add(i, 'days');
    ...
    stopEndDrag = (ev: any) => {
        ...
        if (count !== 0 && cellUnit !== CellUnits.Hour && config.displayWeekend === false) {
            if (count > 0) {
                ...
                while (true) {
                    i++;
                    let tempEnd = moment(eventItem.end).add(i, 'days');
